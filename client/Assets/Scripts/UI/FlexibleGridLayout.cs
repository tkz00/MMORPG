...
public class FlexibleGridLayout : LayoutGroup
{
    public enum FitType
    {
        ...
        UNIFORM,
        // I'm not sure what width and height do
        WIDTH,
        HEIGHT,
        FIXEDROWS,
        FIXEDCOLUMNS
    ...
    [Tooltip("Only works for fixed rows and fixed columns")]
    public bool equalHeightAndWidth;
    ...
    public override void CalculateLayoutInputHorizontal()
    {
        ...
        if (equalHeightAndWidth)
        {
            ...
            {
                cellHeight = parentHeight / (float)rows - ((spacing.y / (float)rows) * (rows - 1))
                                - (padding.top / (float)rows) - (padding.bottom / (float)rows);
                cellWidth = cellHeight;
            }
            else if (fitType == FitType.FIXEDCOLUMNS)
            {
                cellWidth = parentWidth / (float)columns - ((spacing.x / (float)columns) * (columns - 1))
